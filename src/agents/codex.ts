...
/** Gateway origin plus Codex's /v1 path; explicit origins gain the suffix. */
function codexBaseUrl(explicit?: string): string {
  const base = (explicit ?? MANIFEST_BASE_URL).replace(/\/+$/, "");
...
export const codexAdapter: AgentAdapter = {
  ...
  async sessionLaunch(input: SessionLaunchInput) {
    ...
    return {
      ...
      args: [
        ...
        "-c",
        `model_providers.aiand.base_url="${codexBaseUrl(input.baseUrl)}"`,
        "-c",
  ...
  async enable(input: EnableInput) {
    ...
    const next = patchRouting(withDefaults, {
      ...
      baseUrl: codexBaseUrl(input.baseUrl),
      apiKey: input.apiKey,
    });
  ...
  async disable() {
    ...
    // catalog file is ours alone and simply removed. The models cache is only
    // ours to drop while the config still routes through aiand (forced-off /
    // deleted backup) — otherwise it is the user's pre-aiand file that the
    // snapshot just restored, and deleting it would destroy their data.
    await rm(codexCatalogFile(), { force: true });
    const configText = await readTextIfExists(codexConfigFile());
    if (configText.trim() === "") return;
    if (inspectConfig(configText).active) {
      await rm(codexCacheFile(), { force: true });
    }
  },
