...
- API key        -> ItemTable row `secret://cursorAuth/openAIKey`, an Electron
                     ...
                     plaintext `cursorAuth/openAIKey` cell — written only when
                     the encrypted cell can't be produced on this machine.
...
/** Gateway origin plus Cursor's /v1 path; explicit origins gain the suffix. */
function cursorBaseUrl(explicit?: string): string {
  const base = (explicit ?? CURSOR_BASE_URL).replace(/\/+$/, "");
 ...
 * The key goes to the encrypted `secret://` cell modern Cursor reads. Only
 * when the encrypted cell can't be produced (Cursor's Safe Storage key isn't
 * in the keychain yet — e.g. Cursor never launched) does the legacy plaintext
 * cell get written, so `on` still works for older Cursor builds and the test
 * seam. Modern Cursor ignores the plaintext cell once the `secret://` cell
 * exists, and a stale plaintext row could re-arm routing after `off` — so it
 * is never written alongside the encrypted one.
...
async function cursorKeyWrites(
  ...
  if (isSecretEncryptionAvailable({ variant: "cursor", localStatePath })) {
    ...
    writes.push({ op: "set", key: CURSOR_AUTH_OPENAI_KEY, value: apiKey });
  }
  return writes;
}
...
async function enable(
  ...
  let next = setOpenAiBaseUrl(blob, cursorBaseUrl(input.baseUrl));
...
async function probe(): Promise<ProbeResult> {
  ...
  const openAIBaseUrl = blob.openAIBaseUrl;
  // Mirror enable: the default const, or the configured origin (AIAND_BASE_URL)
  // with Cursor's /v1 path. A --base-url flag value is per-invocation, so a
  // flagged `on` followed by unflagged `status` still reports off.
  const configured = cursorBaseUrl(process.env.AIAND_BASE_URL);
  const active = openAIBaseUrl === CURSOR_BASE_URL || openAIBaseUrl === configured;
