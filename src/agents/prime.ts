...
export const primeAdapter: AgentAdapter = {
  ...
  detect(): DetectResult {
    // The official installer ships `prime-agent`; keep `prime` as a legacy
    // fallback. `bin` stays the canonical name because run-agent spawns it.
    const direct = detectBinary(this.bin);
    if (direct.installed) return direct;
