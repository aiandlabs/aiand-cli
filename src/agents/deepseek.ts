...
/** Gateway origin plus DeepSeek's /v1 path; explicit origins gain the suffix. */
function deepseekBaseUrl(explicit?: string): string {
  const base = (explicit ?? BASE_URL).replace(/\/+$/, "");
...
export function patchDeepseekSettings(
  ...
  opts: { modelId: string; models: Model[]; baseUrl?: string },
  ...
  providers[PROVIDER_ID] = {
    ...
    baseURL: deepseekBaseUrl(opts.baseUrl),
