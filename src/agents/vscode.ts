...
/**
 * VS Code holds state.vscdb in memory while running and rewrites it on exit,
 * clobbering anything written underneath it — so both `on` and `off` must
 * land while VS Code is stopped. Mirrors cursor's quit guard; guarded on
 * every platform VS Code runs.
 */
async function vscodeQuitGuard(opts: { force: boolean }): Promise<void> {
...
export const vscodeAdapter: AgentAdapter = {
  ...
  // refresh path.
  enableGuard: vscodeQuitGuard,
  // `off` restores state.vscdb byte-for-byte; a running VS Code rewrites it
  // from memory on exit and would undo the restore. (Unlike cursor, the
  // disable strip deletes our secret rows, so it must also land while
  // stopped — the running app would resurrect them from memory.)
  offGuard: vscodeQuitGuard,
  disable,
};