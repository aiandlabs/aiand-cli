 ...
 * inference.
 */
/** Gateway origin plus Grok's /v1 path; explicit origins gain the suffix. */
function grokBaseUrl(explicit?: string): string {
  const base = (explicit ?? "https://api.aiand.com/v1").replace(/\/+$/, "");
...
export const grokAdapter: AgentAdapter = {
  ...
  async sessionLaunch(input: SessionLaunchInput): Promise<SessionLaunch> {
    ...
    const env = buildGrokLaunchEnvironment({
      apiKey: input.apiKey,
      authPath,
      baseUrl: grokBaseUrl(input.baseUrl),
