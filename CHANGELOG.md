...
- `AIAND_NO_BROWSER=1` never opens a browser; sign-in prints the approval
  URL instead (SSH sessions, headless boxes).
...
- Re-running `install.sh` or `install.ps1` to update an existing install
  no longer stops with "has local changes" over the installer's own
...
- A 200 HTML Gateway body is a 502 on streaming `run`/`chat`, Device
  ...
  through to Device login).
- Terminal hyperlinks (OSC 8) escape the full C0/C1 control range.
...
- Agent setup for OpenCode: `aiand opencode on` wires the stock `opencode`
  binary to ai& permanently; `off` removes exactly what `on` added.
- `aiand run-agent opencode` launches OpenCode on ai& for one session only,
  with nothing written to disk.
- `aiand init` detects installed agents and wires the ones you pick.
- Browser sign-in is the default interactive `aiand login`.
- `aiand opencode on` writes the `aiand` provider (key literal,
  `@ai-sdk/openai-compatible` adapter) into
  ...
  `off` keeps the user's own edits.
- `aiand run-agent opencode [--model <id>] [--] [args…]`: routing and the
  session key are injected into that process's environment, and the agent's
  own exit status is propagated. Works without a prior `on`.
- `aiand init` (`--all`, `--off`, interactive picker): detection never
  installs anything, it prints the official install command instead. The
  picker is an arrow-key, space-to-toggle checkbox over the detected agents,
  with not-installed agents and their install commands listed below. A
  non-interactive bare `aiand init` with nothing detected says so.
  `aiand init --off <agent>` is the same as `aiand <agent> off`.
- Browser sign-in opens the gateway's authorize page and catches the
  redirect on a loopback port (authorization-code + PKCE). Multi-org
  accounts pick their organization with an arrow-key prompt; minted keys are
  labeled `aiand@<hostname>` so the key list names the machine. When the
  gateway has no browser flow (the authorize probe answers 404/501) the CLI
...
- `aiand login --paste` / `--with-token` — sign in with a Pasted key
  (validated against the API before storing). Pasted keys are never revoked
  by `aiand logout`; device-minted keys are. Multi-org accounts pick their
  organization the same way a minted sign-in does.
...
- After an upgrade, the CLI prints a few "what's new" lines from this
  changelog (interactive terminals only).

### Changed

- Node runtime floor is Node 22 or newer (`engines.node` `>=22.0.0`), in
  the CLI and both installers.
...
- `aiand status` distinguishes three auth states: signed in (exit 0),
  ...
  no longer false-fail during an outage. `whoami` still fails loudly on an
  unreachable gateway.
- `status` reports on/off only; it no longer reports other tools writing
  to an agent's config. `on` still refuses a colliding `provider.aiand`
  block without the `x-aiand` Marker.
...
- Banner replaced with the `ai&` ASCII wordmark; `aiand banner`, `help`, and
  `--help` print the wordmark and the version line only.

### Removed

- `aiand login --api-key <sk-...>`: one of the paste flags for the same
  flow (`--paste` prompts masked, `--with-token` reads stdin). Piped keys
  keep working via `--with-token`.
- `aiand login --no-browser` and the SSH/WSL browser detection (including
  clipboard copy of the approval URL). The CLI now just tries the platform
  opener; when none exists (bare WSL, SSH) it prints the URL. In WSL the
  Windows browser opens `localhost` callbacks natively, so detection only
  ever disabled a flow that worked. The non-interactive device path remains
  the automatic fallback when no TTY is present.

### Fixed

- Device-API requests (device code, token poll, refresh, revoke) shipped an
  empty body; the body is now forwarded with the JSON content-type.
- OpenCode `x-aiand` Marker: a root-level stamp made OpenCode 1.18.15 refuse
  the file (`Unrecognized key: x-aiand`), breaking both `opencode on` and
  `run-agent`. The stamp now lives on `provider.aiand.options`, which the
  schema allows, and `on` migrates a leftover root key. A config counts as
  ours only with that stamp plus an https or loopback URL, so a foreign
...
- `opencode.json` reads accept JSONC: OpenCode documents comments and
  ...
  `on`/`status`. A `,}` inside a string value is preserved.
...
- Piped stdin is honored no matter how the parent provides it: sockets
  (Node `child_process` pipes) and Windows anonymous pipes are accepted,
  so `run` context and `login --with-token` keys are no longer silently
  dropped.
...
- Windows `aiand.cmd` launcher: cmd.exe no longer rejects a Node path under
  `C:\Program Files`.
- Git Bash `aiand` shim from `install.ps1`: Windows paths are converted to
  `/c/...` form so a backslash cannot split the baked Node path, and
  `chmod +x` prefers Git's `bash.exe` over WSL's. `install.sh uninstall`
  also removes `aiand.cmd` and runs it via `cmd.exe` for `init --off` when
  the bash shim is missing.
