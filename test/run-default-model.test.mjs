...
/**
 * Run the CLI with stdin ignored: an open-but-silent stdin pipe would look
 * piped, and readStdin() would wait for EOF (src/cli/stdin.ts).
 */
const cli = (args, overrides = {}) =>
  runCli(args, { env: cliEnv({ NO_COLOR: "1", CI: "1", ...overrides }), stdin: "ignore" });
