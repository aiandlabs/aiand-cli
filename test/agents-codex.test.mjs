...
describe("codex adapter", () => {
  ...
  test("disable() keeps a non-aiand models_cache.json (restored snapshot)", async () => {
    // Post-restore state: the engine already put the user's config + cache
    // back byte-for-byte. disable() must only remove aiand-owned files.
    const codexDir = join(home(), ".codex");
    writeFileSync(join(codexDir, "config.toml"), plantedConfig());
    writeFileSync(join(codexDir, "models_cache.json"), '{"mine":true}\n');
    writeFileSync(join(codexDir, "aiand-models.json"), '{"models":[]}');
    await codexAdapter.disable();
    assert.equal(readFileSync(join(codexDir, "config.toml"), "utf8"), plantedConfig());
    assert.equal(readFileSync(join(codexDir, "models_cache.json"), "utf8"), '{"mine":true}\n');
    assert.throws(() => statSync(join(codexDir, "aiand-models.json")), /ENOENT/);
  });

  test("disable() drops the cache while the config is still aiand-routed", async () => {
    // Forced-off / deleted-backup path: no snapshot restore happened, so the
    // stale cache would shadow a future catalog and must go with it.
    const codexDir = join(home(), ".codex");
    writeFileSync(
